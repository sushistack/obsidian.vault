...
네, 대용량 로그를 실시간에 가깝게 처리하기 위해 ELK 스택(Elasticsearch, Logstash, Kibana)을 중심으로 아키텍처를 구성했습니다. 로그 수집 단계에서는 Filebeat를 사용하여 각 서버에서 발생하는 로그를 실시간으로 수집하고, Logstash로 전달했습니다. Logstash에서는 필터링과 파싱을 통해 데이터를 정제한 후 Elasticsearch에 저장했습니다. Elasticsearch는 분산 검색과 분석 엔진으로서 대용량 데이터를 빠르게 처리할 수 있도록 클러스터링을 구성했고, Kibana를 통해 실시간 모니터링과 시각화를 구현했습니다.

• **꼬리질문 1: 실시간 처리를 구현하면서 가장 큰 도전과제는 무엇이었고, 그것을 어떻게 해결하셨나요?**

  

가장 큰 도전 과제는 데이터 유입량이 많을 때 발생하는 시스템의 병목 현상이었습니다. 이를 해결하기 위해 Elasticsearch의 샤드 수를 조정하고 노드를 추가하여 클러스터의 처리 능력을 확장했습니다. 또한, Logstash에서의 필터링 과정을 최적화하여 불필요한 데이터 처리를 줄였고, 필요한 경우 Kafka를 도입하여 로그 수집 과정에서의 부하를 분산시켰습니다.
