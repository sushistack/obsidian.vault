...
저희는 rabbit mq 를 선택했습니다, 그 이유는 자체적으로 제공하는 
...
네, 개발한 Spark 애플리케이션은 수집된 로그 데이터를 실시간으로 처리하고 분석하기 위한 것입니다. 주요 기능으로는 로그 데이터의 정제, 변환, 그리고 특정 키 또는 조건에 따른 집계 작업이 있습니다. 이를 통해 실시간으로 유저 행동 패턴을 분석하거나 시스템의 이상 징후를 감지할 수 있도록 지원했습니다.
...
Spark를 선택한 이유는 대용량 데이터를 빠르게 처리할 수 있는 인메모리 컴퓨팅 엔진이기 때문입니다. 특히, 실시간 데이터 처리를 위한 Spark Streaming 기능을 활용하여 로그 데이터를 스트리밍 방식으로 처리했습니다. 또한, Spark SQL을 사용하여 데이터 프레임 형태로 데이터를 다루어 코드의 가독성과 유지보수성을 높였습니다.
