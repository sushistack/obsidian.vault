...
제가 티맵에 관심을 가지게 된 계기는 티맵 내비를 사용하면서, 유저 별 요청에 대한 최적화된 경로와 공간 데이터를 활용하여 안정적이고 정확하게 서비스를 할 수 있을까? 라는 궁금증으로 부터 출발했습니다. 또 이런 실시간 성 데이터를 다루는 곳에서 일하고 싶어서 지원하게 되었습니다.
...
왜 빨라졌나?
	1. github action 의 캐시 기능
	2. 빌드 시 병렬 작업
	3. 필요한 파일만 업로드 되도록 로직 추가
...
1. **개발하신 기여 트래킹 시스템의 아키텍처와 각 구성 요소가 어떻게 상호 작용하는지 설명해 주실 수 있나요?**

**답변:**

제가 개발한 기여 트래킹 시스템은 유저의 행동을 추적하여 특정 조건을 만족하면 쿠폰을 제공하는 시스템입니다. 아키텍처는 크게 세 부분으로 나뉩니다:

• **추적 스크립트**: 앱과 외부 사이트에 삽입되어 유저의 방문 정보를 실시간으로 감지합니다. 이 스크립트는 이벤트 발생 시 로그 수집 서버로 데이터를 전송합니다.

• **로그 수집 서버**: 대용량 트래픽을 효율적으로 처리하기 위해 RabbitMQ를 사용하여 메시지를 큐에 저장합니다. 그런 다음, 메시지를 Redis로 전달하여 빠른 데이터 처리를 가능하게 합니다.

• **쿠폰 발급 API**: 유저의 쿠폰 사용 가능 여부를 판단하고, 조건이 충족되면 쿠폰을 발급합니다. 이때 Redis에 저장된 유저의 행동 데이터를 기반으로 판단합니다.

이 세 가지 구성 요소는 유기적으로 연동되어 유저의 행동을 추적하고, 실시간으로 쿠폰 발급 여부를 결정합니다.

• **꼬리질문:** 이 시스템에서 높은 트래픽을 처리하기 위해 Redis와 RabbitMQ를 어떻게 활용하셨나요?
• 꼬리2: 다양한 선택지들이 있었을 것으로 보이는데 RabbitMQ를 선택하신 이유는?

**답변:**

높은 트래픽 상황에서 안정적인 처리를 위해 **RabbitMQ**를 사용하여 메시지 큐를 구현하였습니다. 추적 스크립트에서 발생하는 이벤트를 RabbitMQ에 비동기적으로 전송하여 로그 수집 서버의 부하를 줄였습니다. **Redis**는 인메모리 데이터베이스로서 빠른 데이터 접근이 가능하므로, 실시간으로 유저의 상태 정보를 저장하고 조회하는 데 활용했습니다. 이를 통해 빠른 응답성과 확장성을 확보하였습니다.

• **꼬리질문:** 이 아키텍처를 개발하면서 직면한 주요 도전 과제는 무엇이었으며, 그것을 어떻게 극복하셨나요?

**답변:**

주요 도전 과제는 **대용량 트래픽을 실시간으로 처리**하는 것이었습니다. 이를 극복하기 위해 비동기 처리 구조를 도입하고, RabbitMQ와 Redis를 활용하여 시스템의 성능을 최적화하였습니다. 또한, 코드 최적화와 서버 자원 모니터링을 통해 병목 현상을 최소화하였고, 필요 시 서버를 수평적으로 확장할 수 있도록 설계하였습니다.

2. **앱, 외부 사이트부터 결제까지 유저를 추적하는 스크립트를 개발하셨다고 하셨는데, 유저의 프라이버시와 보안을 어떻게 보장하면서 이 추적을 구현하셨나요?**

**답변:**

보안적인 측면은 저희쪽에서만 식별이 가능한 key값을 활용했습니다, 해당 키값을 통해 유저의 정보를 저희쪽 시스템 내부에서만 확인이 가능하여 외부에 유저의 정보가 노출될일 은 없습니다.

• **꼬리질문:** 로그 수집 서버에서 민감한 데이터를 어떻게 저장하고 관리하셨나요?

로그 수집서버에서 관리하는 데이터는 일반적으로 


3. **광고주의 요청으로 단시간에 기능을 개발해야 했다고 하셨는데, 품질을 저해하지 않으면서 어떻게 프로젝트를 관리하여 촉박한 기한을 맞추셨나요?**

**답변:**

우선적으로 **프로젝트의 핵심 요구사항을 명확히 파악**하고, 필수 기능과 부가 기능을 구분하여 **우선순위**를 정했습니다. 팀 내에서는 애자일 방법론을 적용하여 스프린트 단위로 작업을 분할하고, 지속적인 커뮤니케이션을 통해 진행 상황을 공유하였습니다. 또한, 자동화된 테스트와 코드 리뷰를 통해 품질을 유지하면서 개발 속도를 높였습니다.


• **꼬리질문:** 제한된 시간 내에 시스템을 어떻게 테스트하고 검증하셨는지 공유해 주실 수 있나요?

**답변:**

**자동화된 유닛 테스트**와 **통합 테스트**를 작성하여 핵심 기능이 의도한 대로 동작하는지 검증하였습니다. 또한, CI/CD 파이프라인을 구축하여 코드 변경 시마다 자동으로 테스트가 실행되도록 했습니다. 필요한 경우 QA 팀과 협력하여 **사용자 시나리오 기반 테스트**를 진행하였고, 발견된 이슈는 신속하게 수정하였습니다.


래빗 MQ 를 사용한 이유, 

설계 및 검토 단계에서 대용량의 트래픽을 지연시간을 최소화 하면서 처리할 수 있도록 처리량이 높은 비동기 처리 메시지 브로커를 두자는 의견이 있었고, 해당 의견에는 모두 동의하였습니다.

후보에 있었던 브로커로는 kafka와 rabbit mq가 있었습니다.

저희의 선택에서 가장 중요한 요소는 시간적 리소스의 제한이 었습니다.

1. 추가적인 플러그인 없이 자체적으로 제공하는 DLQ
2. 간단 명령어로 설치가능한 admin 페이지를 제공
3. 수 밀리초의 처리 속도를 가져야 한다.
4. 수평 확장 가능해야 한다.
5. 간단한 설정 및 구축
...
**. 1일 평균 90만 라인의 로그를 실시간에 가깝게 키바나로 처리하셨다고 했는데, 이러한 대용량 로그를 실시간 처리하기 위해 어떤 아키텍처나 기술을 사용하셨나요?**

네, 대용량 로그를 실시간에 가깝게 처리하기 위해 ELK 스택(Elasticsearch, Logstash, Kibana)을 중심으로 아키텍처를 구성했습니다. 로그 전달은 Logstash를 사용했습니다. Logstash에서는 필터링과 파싱을 통해 데이터를 정제한 후 Elasticsearch에 저장했습니다. Elasticsearch는 분산 검색과 분석 엔진으로서 대용량 데이터를 빠르게 처리할 수 있도록 클러스터링을 구성했고, Kibana를 통해 실시간 모니터링과 시각화를 구현했습니다.

• **꼬리질문 2: 시스템의 확장성을 고려하여 어떤 설계를 하셨는지 설명해 주실 수 있나요?**

시스템의 확장성을 위해 수평 확장이 용이한 아키텍처를 설계했습니다. 컨테이너 오케스트레이션 도구인 Kubernetes를 활용하여 서비스의 자동 확장과 복원력을 확보했습니다. 또한, 마이크로서비스 구조를 채택하여 각 컴포넌트를 독립적으로 배포하고 관리할 수 있도록 했습니다. 이는 향후 트래픽 증가나 기능 추가 시에도 시스템의 안정성을 유지하면서 확장이 가능하도록 도와주었습니다.

  

**2. 수집된 로그를 가공 및 집계할 수 있는 Spark 애플리케이션을 개발하셨다고 했는데, 이 애플리케이션의 주요 기능과 역할은 무엇인가요?**

네, 개발한 Spark 애플리케이션은 기록된 각 모듈별 로그파일을 읽어서, 각 로그간의 연결 정보를 토대로 하나의 머지된 로그로 생성하는 로직을 만들었습니다. 이를 통해서 유저의 이동 혹은 해동 정보를 한눈에 파악할 수 있는 지표를 제공할 수 있었습니다.

• **꼬리질문 1: Spark를 선택하신 이유는 무엇이며, Spark의 어떤 기능을 활용하셨나요?**

1. Spark를 선택한 이유는 대용량 데이터를 빠르게 처리할 수 있는 인메모리 컴퓨팅 엔진이기 때문입니다.
2. Spark SQL을 사용하여 데이터 프레임 형태로 데이터를 다루어 코드의 가독성과 유지보수성을 높였습니다.
3. 자체적인 내결함성을 가지고 있어 작업이 실패하더라도 안정적으로 데이터를 처리할 수 있기 때문입니다.

• **꼬리질문 2: 로그 데이터 처리 시 성능 최적화를 위해 어떤 방법을 사용하셨나요?**  

성능 최적화를 위해 여러 가지 방법을 적용했습니다. 첫째, 데이터 파티셔닝을 통해 작업을 병렬로 처리하여 처리 속도를 높였습니다. 둘째, 불필요한 컬럼을 제외하고 필요한 데이터만 처리하여 메모리 사용량을 줄였습니다. 셋째, 캐시 기능을 활용하여 반복적으로 사용되는 데이터를 메모리에 저장함으로써 IO 비용을 절감했습니다.

**3. 유저 행동 패턴 분석과 CS 대응을 위한 로그 추적 등 다양한 용도로 활용할 로그를 집계 처리 후 Elasticsearch로 전달하는 배치 시스템을 개발하셨다고 했는데, 이 배치 시스템의 구조와 작동 방식을 자세히 설명해 주실 수 있나요?**

네, 배치 시스템은 Spring Batch를 기반으로 개발되었으며, 주기적으로 로그 데이터를 수집하고 집계하여 Elasticsearch로 전달하는 역할을 합니다. 시스템은 Reader, Processor, Writer의 구조로 구성되어 있습니다. Reader에서는 HDFS나 데이터베이스에서 로그를 읽어오고, Processor에서는 필요한 데이터 변환 및 집계 작업을 수행합니다. 마지막으로 Writer에서는 처리된 데이터를 Elasticsearch에 저장하여 이후 Kibana에서 조회하고 분석할 수 있도록 했습니다.

• **꼬리질문 1: Spring Batch를 사용하여 배치 작업을 구현하셨는데, Spring Batch의 어떤 기능을 활용하셨나요?**

  

Spring Batch의 Job과 Step 구성을 활용하여 배치 작업의 흐름을 관리했습니다. 트랜잭션 관리, 잡 재시작, 스킵 및 리트라이 등의 기능을 사용하여 안정적인 배치 처리를 구현했습니다. 또한, 멀티스레딩을 통해 병렬 처리를 구현하여 배치 작업의 성능을 향상시켰습니다.

  

• **꼬리질문 2: 로그 데이터를 Elasticsearch로 전달하는 과정에서 발생한 문제나 어려움이 있었다면 어떻게 해결하셨나요?**

  

Elasticsearch로 데이터를 전달하는 과정에서 발생한 주요 문제는 데이터 유실과 인덱싱 성능 저하였습니다. 데이터 유실은 네트워크 문제나 Elasticsearch의 부하로 인해 발생할 수 있는데, 이를 방지하기 위해 Bulk API를 사용하여 데이터를 일괄 전송하고, 실패한 요청에 대해서는 재시도 로직을 구현했습니다. 인덱싱 성능 저하는 Elasticsearch의 설정을 조정하고, 인덱스 템플릿을 최적화하여 해결했습니다. 예를 들어, 리플리카 수를 조정하고, 적절한 샤드 수를 설정하여 성능을 개선했습니다.
...
• **동시성:** 한 사람이 여러 가지 일을 동시에 처리하는 것처럼, 전화 통화를 하면서 이메일을 확인하고, 문서를 작성하는 상황.

• **병렬성:** 여러 사람이 각자 다른 일을 동시에 처리하는 것처럼, 한 사람이 전화 통화를 하고 다른 사람이 문서를 작성하는 상황.